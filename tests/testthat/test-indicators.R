...
test_that("Can deal with a bogus indicator", {
  ind_uid <- "abc123"
  de1 <- "aFp9qn4JW5y"
  coc1 <- "TQPl2RZmz5b"
  coc2 <- "KLH5fjVMgKG"

  numerator_formula <- paste0(paste0("#{", de1, ".", coc1, "}"), "+", paste0("#{", de1, ".", coc2, "} + foo.bar"))
  ...
  expect_setequal(names(test_values), c("id", "name", "numerator", "denominator", "value"))
  expect_equal(NROW(test_values), 1)
  expect_equal(test_values$value, NA_real_)
  expect_equal(test_values$numerator, NA_real_)
  expect_equal(test_values$denominator, 1)
}
