...
with_mock_api({
  test_that("We can write an COP25 Target Setting tool", {
    ...
    spectrum_data <-
      readRDS(test_sheet("COP25_spectrum_data_random_MW.rds"))

    d <-
      packTool(
        model_data_path = test_sheet("COP25_datapack_model_data_random_MW.rds"),
        tool = "Data Pack",
    ...
    expect_setequal(class(d_opened$data$MER),
    ...
    expect_setequal(class(d_opened$data$SUBNAT_IMPATT),
                    c("tbl_df", "tbl", "data.frame"))
    expect_identical(unname(sapply(d_opened$data$SUBNAT_IMPATT, typeof)), c(rep("character", 7), "double"))
    expect_setequal(names(d_opened$data$SUBNAT_IMPATT),
                    d_data_targets_names)
    ...
    expect_true(all(unlist(
      purrr::map(sheet_psnus[names(sheet_psnus) != "Prioritization"], identical, wanted_tsnus)
    )))

    #There should be no zeros in d$data$SNUxIM except for dedpe
    expect_false(any(d$data$SNUxIM[d$data$SNUxIM$value == 0 &
                                     !grepl("^0000[01]", d$data$SNUxIM$mech_code), ]))
