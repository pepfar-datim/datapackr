...
with_mock_api({
  test_that("We can create a COP21 OPU Data Pack Schema", {
    ...
    template_file <- getTemplate("COP21_OPU_Data_Pack_Template.xlsx")
...
with_mock_api({
  test_that("We can create a COP22 OPU DataPack Schema", {
    ...
    template_file <- getTemplate("COP22_OPU_Data_Pack_Template.xlsx")
    expect_true(file.exists(template_file))
    test_dataset <- unPackSchema(
      template_path = template_file,
...
with_mock_api({
  test_that("We can create a COP22 DataPack Schema", {
    ...
    template_file <- getTemplate("COP22_Data_Pack_Template.xlsx")
    expect_true(file.exists(template_file))
    test_dataset <- unPackSchema(
      template_path = template_file,
      cop_year = 2022)
    ...
    expect_type(test_dataset$FY, "double")
    expect_type(test_dataset$period, "character")
    load(getRDA("cop22_data_pack_schema.rda"))
...
with_mock_api({
  test_that("We can create a COP23 DataPack Schema", {
    ...
    expect_type(test_dataset$FY, "double")
    expect_type(test_dataset$period, "character")
    load(getRDA("cop23_data_pack_schema.rda"))
