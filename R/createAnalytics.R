...
#' @export
#'
getFY22Prioritizations <- function(d, d2_session = dynGet("d2_default_session", inherits = TRUE)) {

  psnu_prioritizations <- d$datim$fy22_prioritizations %>%
    dplyr::select(orgUnit, value)

  psnus <- datapackr::getPSNUs(country_uids = d$info$country_uids,
                               include_mil = FALSE, include_DREAMS = FALSE,
...
createAnalytics <- function(d,
  ...
  if (d$info$tool == "OPU Data Pack") {
    ...
    if (d$info$cop_year %in% c(2021, 2022)) {
      ...
      prioritizations <- getFY22Prioritizations(d, d2_session = d2_session)
