...
checkHasPSNUxIM <- function(d) {
  ...
  if (NROW(d$data$SNUxIM) == 1 && is.na(d$data$SNUxIM[[1, 1]])) {
    ...
    d$info$messages <- appendMessage(d$info$messages, warning_msg, "WARNING", "PSNUxIM")
...
extractDuplicateRows <- function(d, sheet = "PSNUxIM") {
  ...
  if (NROW(duplicates) > 0) {
    ...
    d$info$messages <- appendMessage(d$info$messages, warning_msg, "ERROR", "PSNUxIM")
...
checkPSNUxIMDisaggs <- function(d) {
  ...
  if (any(is.na(defunct_disaggs$exists))) {
    ...
    d$info$messages <- appendMessage(d$info$messages, msg, lvl, "PSNUxIM")
    d$info$has_error <- TRUE
  }
