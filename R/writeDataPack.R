...
Nigeria_PSNU_Patch <- function(d) {
    print("Applying Nigeria PSNU filtering patch")

    Nigeria_patch_file <- paste0(d$keychain$support_files_path,
                                 if (is.na(stringr::str_extract(d$keychain$support_files_path,"/$"))) {"/"} else {},
                                 "Nigeria COP19 SNUs Not PEPFAR Supported.csv")

    nigeria_filter <- readr::read_csv(Nigeria_patch_file) %>%
        dplyr::mutate(psnuid = stringr::str_extract(PSNU,"(?<=\\()([A-Za-z][A-Za-z0-9]{10})(?=\\)$)"))

    PSNUs <- d$data$PSNUs %>%
        dplyr::left_join(nigeria_filter, by = c("uid" = "psnuid")) %>%
        dplyr::filter(is.na(PSNU)) %>%
        select(-PSNU, -IMPATT.PRIORITY_SNU.20T)
...
getPSNUs <- function(d) {
    ...
    # Pull PSNUs from DATIM SQL view
        ...
        PSNUs <- paste0(getOption("baseurl"),
                        "api/sqlViews/PjjAyeXUbBd/data.json?fields=operating_unit,uid,name&filter=name:!like:_Military&filter=operating_unit:",
    ...
    # Grab PSNU list
        ...
        ## DRC Patch
            if (d$data$ou_name == "Democratic Republic of the Congo") {
                d$data$PSNUs <- DRC_PSNU_Patch(d)
            }

        ## Nigeria Patch
            if (d$data$ou_name == "Nigeria") {
                d$data$PSNUs <- Nigeria_PSNU_Patch(d)
            }
